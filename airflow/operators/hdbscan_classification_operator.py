...
class EarthquakeClusterClassifier:
    """
    Classification operator untuk prediksi risk level dari earthquake clusters
    menggunakan Random Forest dan XGBoost dengan evaluasi comprehensive
    """
    ...
    def read_training_data(self) -> Optional[pl.DataFrame]:
        """
        Read earthquake data with clustering results and spatial features
        for classification training
        
        NOTE: Includes BOTH cluster-assigned (cluster_id != -1) AND noise points (cluster_id = -1)
        to match the risk_label distribution from clustering task and enable VERY_HIGH prediction
        """
        try:
            if not self.conn:
                return None
    ...
    def prepare_features(self, df: pl.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Features:
        - magnitude, depth (earthquake characteristics)
        - nearest_event_km, event_density_100km (spatial)
        - hdbscan_probability (clustering confidence)
        - latitude, longitude (location)
        
        Target: risk_label (LOW, MEDIUM, HIGH, VERY_HIGH)
        """
        try:
    ...
    def evaluate_model(self, model, X_test: np.ndarray, y_test: np.ndarray, model_name: str) -> Dict:
        ...
        try:
            ...
            # Calculate WEIGHTED metrics (for imbalanced multiclass)
            ...
            # Confusion matrix with explicit labels to include all classes
    ...
    def create_comparison_plots(self, rf_results: Dict, xgb_results: Dict) -> Dict[str, Path]:
        """Create separate comparison plots between RF and XGBoost"""
        try:
            logger.info("Creating comparison plots (separate)...")
            output_paths = {}
            
            # 1. Model Performance Comparison (Metrics Bar Chart)
    ...
    def create_earthquake_distribution_chart(self, df: pl.DataFrame) -> Path:
        """Create earthquake distribution chart by year"""
        try:
            logger.info("Creating earthquake distribution by year chart...")
            
            # Extract year from datetime or time column
            # Try multiple column names
            date_col = None
            for col_name in ['datetime', 'time', 'date']:
                if col_name in df.columns:
                    date_col = col_name
                    break
            
            if date_col is None:
                logger.warning("No date column found, skipping year distribution chart")
                return None
            ...
            year_dist = df_temp.group_by('year').agg(
            ...
            for _, row in year_data.iterrows():
                logger.info(f"  Year {int(row['year'])}: {int(row['count']):6d} earthquakes")
            ...
            bars = ax.bar(years, counts, color='#E67E22', alpha=0.8, edgecolor='#D35400', linewidth=1.5)
            
            ax.set_xlabel('Year', fontweight='bold', fontsize=12)
            ax.set_ylabel('Number of Earthquakes', fontweight='bold', fontsize=12)
            ax.set_title('Earthquake Records Distribution by Year (Asia Region)', fontweight='bold', fontsize=14)
            ax.grid(axis='y', alpha=0.3)
    ...
    def run_classification_pipeline(self) -> Dict:
        ...
        try:
            ...
            # Create earthquake distribution by year
...
def run_classification_task(**context):
    """Airflow task function untuk menjalankan classification pipeline"""
    try:
